...
export function exportCss(result: SeatResult, options: ExportOptions = {}): string {
  const bang = options.important ? " !important" : "";
  ...
  for (const block of result.blocks) {
    ...
    if (block.leadingTo !== block.leadingFrom) {
      declarations.push(`  line-height: ${block.leadingTo}px${bang};`);
    }
    if (block.lever === "padding" && block.paddingAdded > 0.01) {
      declarations.push(`  padding-top: ${block.paddingTop}px${bang};`);
    }
    if (block.lever === "offset" && block.offset !== 0) {
      ...
      declarations.push(`  position: relative${bang};`, `  top: ${block.offset}px${bang};`);
    }
